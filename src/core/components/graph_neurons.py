...
class RelationalRouter(hk.Module):
    ...
    def __call__(self, features: jnp.ndarray, adjacency: jnp.ndarray) -> jnp.ndarray:
        """
...
class GraphMoE(hk.Module):
    ...
    def __call__(
        ...
        if self.use_relational_routing and adjacency is not None:
            router = RelationalRouter(self.d_model, self.num_experts, name="relational_router")
...
class GraphIntegratedTransformerBlock(hk.Module):
    ...
    def __call__(
        ...
        if self.use_graph_neurons:
            ...
            graph_builder = DynamicGraphBuilder(self.d_model, self.max_nodes, name="graph_builder")
...
def compute_graph_loss(
    predicted_adjacency: jnp.ndarray, target_adjacency: jnp.ndarray, edge_weight: float = 1.0
) -> float:
    """

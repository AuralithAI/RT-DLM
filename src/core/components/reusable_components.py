...
class ReusableAttention(hk.Module):
    ...
    def __init__(
        ...
        # Spiking mechanism
        self.spiking = SpikingMechanism(self.default_spike_threshold, self.default_epsilon)
    ...
    def get_head_usage(self) -> jnp.ndarray:
        ...
        return hk.get_state("head_usage", [self.num_heads], dtype=jnp.float32, init=jnp.zeros)
...
class ReusableFeedForward(hk.Module):
    ...
    def __call__(self, inputs: jnp.ndarray, track_usage: bool = True) -> jnp.ndarray:
        """
    ...
    def _update_usage(self, ffn_out: jnp.ndarray) -> None:
        ...
        current_usage = hk.get_state("ffn_usage", [self.d_model], dtype=jnp.float32, init=jnp.zeros)
...
def create_attention(
    d_model: int = 256, num_heads: int = 8, spike_threshold: float = 0.1, name: Optional[str] = None
) -> ReusableAttention:
    """
    ...
    """
    config = AttentionConfig(d_model=d_model, num_heads=num_heads, spike_threshold=spike_threshold)
...
def create_transformer_block(
    ...
    """
    config = AttentionConfig(d_model=d_model, num_heads=num_heads, spike_threshold=spike_threshold)
    return ReusableTransformerBlock(config=config, ffn_expansion=ffn_expansion, name=name)
...
def compute_attention_sparsity(scores: jnp.ndarray, spike_threshold: float = 0.1) -> float:
    """

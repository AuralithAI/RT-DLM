...
class SanitizationConfig:
    ...
    replacement_tokens: Dict[PIIType, str] = field(
...
class DataSanitizer:
    ...
    def _get_replacement(self, finding: PIIFinding) -> str:
        ...
        strategy = self.config.type_strategies.get(finding.pii_type, self.config.default_strategy)

        if strategy == RedactionStrategy.TOKEN:
            return self.config.replacement_tokens.get(finding.pii_type, "[REDACTED]")
...
def create_strict_sanitizer() -> DataSanitizer:
    ...
    """
    config = SanitizationConfig(default_strategy=RedactionStrategy.TOKEN, log_sanitization=True)

...
class CrossMemoryAttention(hk.Module):
    ...
    def __call__(
        self, ltm: jnp.ndarray, stm: jnp.ndarray, mtm: jnp.ndarray, is_training: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        ltm_gate = hk.Sequential([hk.Linear(self.d_model), jax.nn.sigmoid], name="ltm_gate")
        stm_gate = hk.Sequential([hk.Linear(self.d_model), jax.nn.sigmoid], name="stm_gate")
        mtm_gate = hk.Sequential([hk.Linear(self.d_model), jax.nn.sigmoid], name="mtm_gate")
...
class HierarchicalMemoryFusion(hk.Module):
    ...
    def _compute_memory_importance(
        self, ltm: jnp.ndarray, stm: jnp.ndarray, mtm: jnp.ndarray, context: jnp.ndarray
    ) -> jnp.ndarray:
        """
    ...
    def __call__(
        ...
        results["level1"] = {"ltm": ltm_local, "stm": stm_local, "mtm": mtm_local}

        # Level 2: Cross-attention between memory banks
        cross_result = self.cross_memory_attention(ltm_local, stm_local, mtm_local, is_training)
        ...
        if context is not None:
            importance = self._compute_memory_importance(ltm_cross, stm_cross, mtm_cross, context)
        else:
...
class InfiniteContextAttention(hk.Module):
    ...
    def _compress_chunk(self, chunk: jnp.ndarray, is_training: bool = True) -> jnp.ndarray:
        """
        ...
        summary_queries = jnp.broadcast_to(summary_queries, (batch_size, num_summary, self.d_model))
    ...
    def _local_attention(
        self, chunk: jnp.ndarray, global_context: jnp.ndarray, is_training: bool = True
    ) -> jnp.ndarray:
        """
...
class AGIAttention(hk.Module):
    ...
    def __init__(self, config: AGIAttentionConfig, name: Optional[str] = None):
        super().__init__(name=name)
        self.config = config
    ...
    def __call__(
        ...
        attn_output, attn_info = self.forward_attention(x, mask, is_training, use_infinite_context)
...
def create_agi_attention(
    ...
    **kwargs,
    ...
    config = AGIAttentionConfig(d_model=d_model, num_heads=num_heads, **base_config)

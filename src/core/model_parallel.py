...
class TensorParallelLinear(hk.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        if self.parallel_mode == "column":
            ...
            if self.with_bias:
                b = hk.get_parameter("b", shape=[local_output_size], init=jnp.zeros)
                output = output + b
...
class TensorParallelAttention(hk.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, mesh: DeviceMesh, name: Optional[str] = None):
        super().__init__(name=name)
    ...
    def __call__(self, x: jnp.ndarray, mask: Optional[jnp.ndarray] = None) -> jnp.ndarray:
        ...
        q_proj = TensorParallelLinear(local_dim, self.mesh, parallel_mode="column", name="query")
        k_proj = TensorParallelLinear(local_dim, self.mesh, parallel_mode="column", name="key")
        v_proj = TensorParallelLinear(local_dim, self.mesh, parallel_mode="column", name="value")
...
class TensorParallelMLP(hk.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        fc1 = TensorParallelLinear(self.d_ff, self.mesh, parallel_mode="column", name="fc1")
        hidden = fc1(x)
        hidden = self.activation(hidden)

        # Second linear: row parallel (reduce across devices)
        fc2 = TensorParallelLinear(self.d_model, self.mesh, parallel_mode="row", name="fc2")
        output = fc2(hidden)
...
class PipelineParallelModel(hk.Module):
    ...
    def __call__(self, input_ids: jnp.ndarray, mask: Optional[jnp.ndarray] = None) -> jnp.ndarray:
        ...
        final_norm = hk.LayerNorm(axis=-1, create_scale=True, create_offset=True, name="final_norm")
        x = final_norm(x)
...
def shard_params_for_tensor_parallel(params: Dict, mesh: DeviceMesh, layer_names: List[str]) -> Dict:
    ...
    param_shardings = jax.tree_util.tree_map_with_path(
        lambda path, p: mesh.get_sharding(get_sharding_for_param(path, p)), params
    )
...
def create_sharded_train_step(train_step_fn: Callable, mesh: DeviceMesh, param_shardings: Dict) -> Callable:
    ...
    @partial(
        jax.jit,
        in_shardings=(param_shardings, None, None, None),
...
class ModelParallelTransformer(hk.Module):
    ...
    def __init__(self, config, mesh: DeviceMesh, name: Optional[str] = None):
        super().__init__(name=name)
        self.config = config
        self.mesh = mesh

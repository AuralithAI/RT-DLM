...
class RLMOrchestrator:
    ...
    def init(self, rng: jnp.ndarray) -> Params:
        def forward(query_emb, context_len, depth, calls):
            ...
            # Run both forward and synthesize_answer to initialize all parameters
            output = model(query_emb, context_len, depth, calls)
            # Also trace synthesize_answer path to initialize its parameters
            dummy_results = [query_emb]  # Use query_emb as a dummy intermediate result
            _ = model.synthesize_answer(query_emb, dummy_results)
            return output

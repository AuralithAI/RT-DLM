...
class ScalableMesh:
    ...
    def _auto_configure(self):
        ...
        if self.config.tensor_parallel:
            self.tensor_parallel_size = min(self.config.tensor_parallel_size, self.num_devices)
        else:
            self.tensor_parallel_size = 1

        if self.config.pipeline_parallel:
            self.pipeline_parallel_size = min(
                self.config.pipeline_parallel_size, self.num_devices // self.tensor_parallel_size
            )
        else:
...
def get_param_sharding_spec(param_name: str, param_shape: Tuple[int, ...], mesh: ScalableMesh) -> P:
    """
...
def create_sharded_params(params: Dict, mesh: ScalableMesh) -> Tuple[Dict, Dict]:
    """
    ...
    param_specs = jax.tree_util.tree_map_with_path(get_spec_for_path, params)

    param_shardings = jax.tree_util.tree_map(lambda spec: mesh.get_sharding(spec), param_specs)
...
def create_scalable_train_step(
    ...
    def train_step_impl(params, opt_state, batch, rng):
        ...
        (loss, outputs), grads = jax.value_and_grad(compute_loss, has_aux=True)(params, batch, rng)
...
def setup_scalable_training(
    ...
    if mesh.has_tensor_parallel:
        ...
        params = jax.tree_util.tree_map(lambda p, s: jax.device_put(p, s), params, param_shardings)
...
def recommend_parallelism(
    model_memory_gb: float, device_memory_gb: float = 16.0, num_devices: int = None
) -> Dict[str, Any]:
    """
...
class DistributedProfiler:
    ...
    def profile_all_reduce(self, tensor: jnp.ndarray, num_iterations: int = 10) -> Dict[str, float]:
        ...
        if not self.mesh.is_distributed:
            return {
                "latency_ms": 0.0,
                "bandwidth_gbps": 0.0,
                "message": "Single device, no communication",
            }
    ...
    def profile_train_step(
        self, train_step_fn: Callable, params, opt_state, batch: Dict, rng, num_iterations: int = 10
    ) -> Dict[str, float]:

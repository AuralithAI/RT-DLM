...
class MixedPrecisionPolicy:
    ...
    def cast_params(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result: Dict[str, Any] = jax.tree_util.tree_map(
            lambda x: x.astype(self.param_dtype) if hasattr(x, "astype") else x, params
        )
        return result
    ...
    def cast_inputs(self, inputs: Dict[str, jnp.ndarray]) -> Dict[str, jnp.ndarray]:
        ...
        result: Dict[str, jnp.ndarray] = jax.tree_util.tree_map(
            lambda x: (
                ...
                if hasattr(x, "astype") and x.dtype in [jnp.float32, jnp.float16, jnp.bfloat16]
                else x
            ),
            inputs,
        )
...
def create_pmap_train_step(train_step_fn: Callable, axis_name: str = "devices") -> Callable:
    """
    ...
    def pmap_train_step(params, opt_state, batch, rng):
        ...
        new_params, new_opt_state, loss, outputs = train_step_fn(params, opt_state, batch, rng)
...
def create_accumulated_train_step(base_train_step: Callable, accumulation_steps: int) -> Callable:
    """
...
def log_memory_usage(prefix: str = ""):
    ...
    for device_name, device_stats in stats.items():
        ...
        if bytes_limit > 0:
            ...
            logger.info(
                f"{prefix}{device_name}: {bytes_used / 1e9:.2f}GB / {bytes_limit / 1e9:.2f}GB ({usage_pct:.1f}%)"
            )

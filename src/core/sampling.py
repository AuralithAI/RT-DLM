...
class TokenSampler:
    ...
    def apply_repetition_penalty(
        self, logits: jnp.ndarray, generated_tokens: jnp.ndarray, penalty: float = 1.0
    ) -> jnp.ndarray:
    ...
    def sample(
        ...
        entropy = -jnp.sum(jnp.where(probs > 1e-10, probs * log_probs, 0.0), axis=-1)
    ...
    def beam_search(
        ...
        def _block_repeated_ngrams(
            logits: jnp.ndarray, sequences: jnp.ndarray, ngram_size: int, current_pos: int
        ) -> jnp.ndarray:
        ...
        for step in range(max_length - initial_seq_len):
            ...
            candidate_scores = jnp.where(
                finished_mask, jnp.full_like(candidate_scores, -float("inf")), candidate_scores
            )
            ...
            for b in range(batch_size):
                for k in range(beam_width):
                    ...
                    new_sequences = new_sequences.at[b, k].set(beam_sequences[b, source_beam])
                    # Add the new token
        ...
        for b in range(batch_size):
            best_sequences = best_sequences.at[b].set(beam_sequences[b, best_beam_indices[b]])
            best_scores = best_scores.at[b].set(normalized_scores[b, best_beam_indices[b]])
...
class SpeculativeDecoder:
    ...
    def __init__(
        ...
        self.config = SamplingConfig(temperature=temperature, top_k=top_k, top_p=top_p)

    def _draft_tokens(
        self, draft_params, tokens: jnp.ndarray, rng_key, num_tokens: int
    ) -> Tuple[jnp.ndarray, jnp.ndarray]:
        """
        ...
        for i in range(num_tokens):
            ...
            # Append to sequence
            current_tokens = jnp.concatenate([current_tokens, draft_token[:, None]], axis=1)
...
class SelfSpeculativeDecoder:
    ...
    def _check_early_exit_support(self, params, sample_tokens: jnp.ndarray) -> bool:
        ...
        try:
            ...
            _ = self.model_forward_fn(params, text=sample_tokens, exit_layer=self.early_exit_layer)
    ...
    def _draft_tokens_early_exit(
        ...
        return (jnp.concatenate(draft_tokens, axis=1), jnp.concatenate(draft_probs, axis=1))
    ...
    def _verify_drafts(
        ...
        full_probs_for_draft = jnp.take_along_axis(full_probs, draft_tokens[:, :, None], axis=-1)[
            :, :, 0
        ]  # [batch, num_draft]

        # Acceptance probability: min(1, full_prob / draft_prob)
        acceptance_ratio = jnp.minimum(1.0, full_probs_for_draft / (draft_probs + 1e-10))
    ...
    def _contains_stop_token(self, tokens: jnp.ndarray, stop_tokens: List[int]) -> bool:
    ...
    def generate(
        ...
        if not self._check_early_exit_support(params, initial_tokens):
            return self._standard_generate(params, initial_tokens, rng_key, max_length, stop_tokens)

        return self._speculative_generate(params, initial_tokens, rng_key, max_length, stop_tokens)

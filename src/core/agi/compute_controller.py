...
class ModuleRegistry:
    ...
    def get_available(self, state: ComputeState, budget: float) -> List[ModuleContract]:
        """
        ...
        for module_type, contract in self._modules.items():
            ...
            # Check dependencies
            deps_met = all(dep in state.modules_called for dep in contract.dependencies)
            if not deps_met:
                continue
    ...
    def get_total_cost(self, modules: List[ModuleType]) -> float:
        ...
        return sum(self._modules[m].base_cost for m in modules if m in self._modules)
    ...
    def get_code_boosted_costs(
        ...
        for module_type, contract in self._modules.items():
            ...
            if code_confidence > threshold and module_type in CODE_MODULES:
...
class ComputeController(hk.Module):
    ...
    def _encode_state(self, state: ComputeState) -> jnp.ndarray:
        ...
        budget_features = budget_encoder(jnp.full((encoded.shape[0], 1), state.budget_remaining))
    ...
    def _encode_module_history(self, state: ComputeState) -> jnp.ndarray:
        ...
        history_encoder = hk.Sequential(
            [hk.Linear(self.d_model), jax.nn.silu, hk.Linear(self.d_model)], name="history_encoder"
        )
        ...
        history_onehot = jnp.broadcast_to(history_onehot[None, :], (batch_size, self.num_modules))
    ...
    def __call__(
        self, state: ComputeState, available_modules: List[ModuleContract], is_training: bool = True
        ...
        halt_predictor = hk.Sequential(
            [hk.Linear(self.d_model // 2), jax.nn.silu, hk.Linear(1), jax.nn.sigmoid],
            name="halt_predictor",
        )
...
class ComputePlan(hk.Module):
    ...
    def _get_all_gate_params(self, hidden_pooled: jnp.ndarray) -> Tuple[List[jnp.ndarray], List[jnp.ndarray]]:
        ...
        for i in range(self.max_steps):
            ...
            b = hk.get_parameter(f"update_gate_{i}_b", shape=[self.d_model], init=jnp.zeros)
            weights.append(w)
            biases.append(b)
    ...
    def update_state(
        self, state: ComputeState, module_output: ModuleOutput, module_type: ModuleType, cost: float
    ) -> ComputeState:
    ...
    def apply_think_budget(
        ...
        """Compute effective max_steps and initial_budget from a think-budget.

        The think-budget is a token count that maps to controller steps:
        - More tokens → more controller steps → more reasoning
        - Budget scales proportionally so the controller has room to use
          the extra steps without running out of budget.

        Preset mapping (from run_eval THINK_BUDGET_PRESETS):
            low=256  → scale ~0.25x  (fewer steps, faster)
            medium=1024 → scale 1.0x (default)
            high=4096 → scale 4.0x  (more steps, deeper reasoning)
            max=8192 → scale 8.0x  (maximum reasoning)

        Args:
            think_budget_tokens: Token budget for reasoning. None = use defaults.

        Returns:
            (effective_max_steps, effective_budget)
        """
        if think_budget_tokens is None:
            return self.max_steps, self.initial_budget

        # Reference: medium=1024 maps to scale=1.0
        REFERENCE_TOKENS = 1024
        scale = max(0.1, think_budget_tokens / REFERENCE_TOKENS)

        effective_steps = max(1, int(self.max_steps * scale))
        effective_budget = min(10.0, self.initial_budget * scale)

        return effective_steps, effective_budget
    ...
    def __call__(
        ...
        Args:
            ...
            think_budget_tokens: Optional think-budget override (token count).
                When set, scales max_steps and budget proportionally.
                Low (256) → fewer steps; High (4096) → more reasoning.

        Returns:
            final_state: Final compute state
            execution_info: Dictionary with execution trace
        """
        # Apply think-budget override
        effective_steps, effective_budget = self.apply_think_budget(think_budget_tokens)

        # Initialize state (with possibly scaled budget)
        state = self.initialize_state(hidden, memory_summary)
        # Override budget if think-budget was set
        if think_budget_tokens is not None:
            state = state._replace(budget_remaining=effective_budget)
        ...
        # Main loop — use effective_steps (possibly scaled by think-budget)
...
def create_compute_controller_fn(
    d_model: int, max_steps: int = 10, initial_budget: float = 1.0, halt_threshold: float = 0.8
):
    """
...
class ControllerLossComputer:
    ...
    def compute_utilization_loss(
        self, modules_called: List[ModuleType], total_available: int = len(ModuleType)
    ) -> jnp.ndarray:
        """
    ...
    def compute_ponder_loss(self, halt_probs: List[jnp.ndarray]) -> jnp.ndarray:
        """
...
class ControllerRewardShaper:
    ...
    def compute_returns(self, step_rewards: List[float], final_reward: float) -> List[float]:
...
class GRPOValueHead(hk.Module):
    ...
    def __init__(self, d_model: int, dropout_rate: float = 0.0, name: Optional[str] = None):
        super().__init__(name=name)
        self.d_model = d_model
        self.dropout_rate = dropout_rate

    def __call__(self, hidden_state: jnp.ndarray, is_training: bool = True) -> jnp.ndarray:
        """
        ...
        x = jax.nn.silu(x)
        x = hk.LayerNorm(axis=-1, create_scale=True, create_offset=True, name="value_ln")(x)

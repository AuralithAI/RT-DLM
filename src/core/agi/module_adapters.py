...
from src.core.agi.compute_controller import ModuleType, ModuleOutput, ComputeState
...
class ModuleAdapter(hk.Module):
    ...
    def __init__(self, d_model: int, module_type: ModuleType, name: Optional[str] = None):
        ...
        self.confidence_estimator = hk.Sequential(
            [hk.Linear(d_model // 2), jax.nn.silu, hk.Linear(1), jax.nn.sigmoid],
        ...
        self.halt_suggester = hk.Sequential(
            [hk.Linear(d_model // 4), jax.nn.silu, hk.Linear(1), jax.nn.sigmoid],
            name="halt_suggester",
        )
    ...
    def _compute_uncertainty(self, output: jnp.ndarray, input_state: jnp.ndarray) -> jnp.ndarray:
    ...
    def _compute_delta(self, output: jnp.ndarray, input_state: jnp.ndarray) -> jnp.ndarray:
    ...
    def __call__(
        ...
        if self.graph_module is not None:
            ...
            raw_output = self.graph_module(
                node_features=hidden, adjacency=adjacency, query=query, is_training=is_training
            )
        else:
            ...
            raw_output = {"output": current, "reasoning_trace": jnp.stack(reasoning_trace, axis=1)}
...
class SymbolicReasoningAdapter(ModuleAdapter):
    ...
    def __call__(
        self, state: ComputeState, rules: Optional[jnp.ndarray] = None, is_training: bool = True
    ) -> ModuleOutput:
...
class ProbabilisticAdapter(ModuleAdapter):
    ...
    def __call__(self, state: ComputeState, is_training: bool = True) -> ModuleOutput:
    ...
    def __call__(self, state: ComputeState, is_training: bool = True) -> ModuleOutput:
        ...
        if self.moe_module is not None:
            ...
            if isinstance(raw_output, tuple):
                raw_output = {
                    "output": raw_output[0],
                    "aux_loss": raw_output[1] if len(raw_output) > 1 else 0.0,
                }
        else:
        ...
        evidence = {"num_experts": self.num_experts, "top_k": self.top_k}
...
class ConsciousnessAdapter(ModuleAdapter):
    ...
    def __call__(
        ...
        if self.consciousness_module is not None:
            ...
            raw_output = {"introspection": introspected, "meta_awareness": meta, "output": meta}
...
def create_module_executors(
    d_model: int, vocab_size: int, existing_modules: Optional[Dict[str, hk.Module]] = None
    ...
    probabilistic_adapter = ProbabilisticAdapter(
        d_model=d_model, probabilistic_module=existing_modules.get("probabilistic")
    )

    moe_adapter = MoERoutingAdapter(d_model=d_model, moe_module=existing_modules.get("moe"))

    consciousness_adapter = ConsciousnessAdapter(
        d_model=d_model, consciousness_module=existing_modules.get("consciousness")
    )

    output_adapter = OutputGenerationAdapter(
        d_model=d_model, vocab_size=vocab_size, output_head=existing_modules.get("output_head")
    )

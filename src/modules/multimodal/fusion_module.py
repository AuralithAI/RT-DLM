...
class VideoEncoder(hk.Module):
    ...
    def __call__(self, video_frames):
        ...
        for layer in self.temporal_layers:
            temporal_features = self.norm(
                temporal_features + layer(temporal_features, temporal_features, temporal_features)
            )
...
class MultiModalRTDLM(hk.Module):
    ...
    def __init__(self, config, name=None):
        ...
        self.fusion_layer = MultiModalFusionLayer(
            config.d_model, config.num_heads, modalities=["text", "vision", "audio", "video"]
        )

        # Final projection layers
        self.output_projection = hk.Linear(config.vocab_size, name="output_proj")
        self.modality_classifier = hk.Sequential(
            [hk.Linear(config.d_model), jax.nn.silu, hk.Linear(4), jax.nn.softmax],  # 4 modalities

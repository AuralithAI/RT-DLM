...
class ConvNeXtBlock(hk.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        # Feedforward network
        ffn = hk.Sequential(
            [hk.Linear(self.out_channels * 4), jax.nn.gelu, hk.Linear(self.out_channels)],
            name="ffn",
        )
...
class RNNAudioEncoder(hk.Module):
    ...
    def __call__(self, spectrogram: jnp.ndarray) -> jnp.ndarray:
        ...
        attended_output = self.temporal_attention(conv_output, conv_output, conv_output)
...
class SpeechRecognitionModule(hk.Module):
    ...
    def __init__(self, d_model: int, name=None):
        ...
        self.phoneme_detector = hk.Sequential(
            [hk.Linear(d_model), jax.nn.silu, hk.Linear(128), jax.nn.softmax],  # Phoneme classes
            name="phoneme_detector",
        )

        # Speech activity detector
        self.speech_detector = hk.Sequential(
            [hk.Linear(d_model), jax.nn.silu, hk.Linear(1), jax.nn.sigmoid], name="speech_detector"
        )

        # Speaker embedding
        self.speaker_encoder = hk.Sequential(
            [hk.Linear(d_model), jax.nn.silu, hk.Linear(256)],  # Speaker embedding
...
class MusicAnalysisModule(hk.Module):
    ...
    def __init__(self, d_model: int, name=None):
        ...
        self.genre_classifier = hk.Sequential(
            [hk.Linear(d_model), jax.nn.silu, hk.Linear(10), jax.nn.softmax],  # Music genres
            name="genre_classifier",
        )

        # Beat tracker
        self.beat_tracker = hk.Sequential(
            [hk.Linear(d_model), jax.nn.silu, hk.Linear(1), jax.nn.sigmoid], name="beat_tracker"
        )
...
class AudioEmotionModule(hk.Module):
    ...
    def __init__(self, d_model: int, name=None):
        ...
        self.valence_predictor = hk.Sequential(
            [hk.Linear(d_model), jax.nn.silu, hk.Linear(1), jax.nn.tanh],  # -1 to 1
            name="valence_predictor",
        )

        # Arousal predictor (calm/excited)
        self.arousal_predictor = hk.Sequential(
            [hk.Linear(d_model), jax.nn.silu, hk.Linear(1), jax.nn.sigmoid],  # 0 to 1

...
def compute_ewc_loss(
    ...
    param_losses = jax.tree_util.tree_map(compute_param_loss, params, params_star, fisher_matrix)

    # Sum all parameter losses
    ewc_loss = jax.tree_util.tree_reduce(lambda x, y: x + y, param_losses)
...
class SynapticIntelligence(hk.Module):
    ...
    def __init__(self, d_model: int, lambda_si: float = 1.0, damping: float = 0.1, name=None):
        ...
        self.path_integrator = hk.Sequential(
            [hk.Linear(d_model), jax.nn.silu, hk.Linear(d_model)], name="path_integrator"
        )
...
class ProgressiveNeuralNetwork(hk.Module):
    ...
    def __init__(self, d_model: int, max_columns: int = 10, name=None):
        ...
        for i in range(max_columns - 1):
            self.lateral_adapters.append(hk.Linear(d_model, name=f"lateral_{i}"))

        # Output projection per column
        self.column_outputs = []
        for i in range(max_columns):
            self.column_outputs.append(hk.Linear(d_model, name=f"column_out_{i}"))

    def __call__(
        self,
        x: jnp.ndarray,
        column_idx: int = 0,
        previous_outputs: Optional[List[jnp.ndarray]] = None,
    ) -> jnp.ndarray:
        """
...
class ContinualLearner(hk.Module):
    ...
    def __init__(
        ...
        self.importance_network = hk.Sequential(
            [hk.Linear(d_model * 2), jax.nn.silu, hk.Linear(d_model), jax.nn.sigmoid],
    ...
    def __call__(
        ...
        consolidation_input = jnp.concatenate([features, features * importance_weights], axis=-1)
...
class OnlineMetaLearner(hk.Module):
    ...
    def __init__(self, d_model: int, inner_lr: float = 0.01, name=None):
        ...
        self.knowledge_transfer = hk.Sequential(
            [hk.Linear(d_model * 2), jax.nn.silu, hk.Linear(d_model)], name="knowledge_transfer"
        )
...
class MemoryAwareSynapses(hk.Module):
    ...
    def __init__(self, d_model: int, damping: float = 0.01, name=None):
        ...
        self.sensitivity_estimator = hk.Sequential(
            [hk.Linear(d_model), jax.nn.silu, hk.Linear(d_model), jax.nn.sigmoid],
            name="sensitivity_estimator",
        )

        # Memory importance network
        self.importance_net = hk.Sequential(
            [hk.Linear(d_model * 2), jax.nn.silu, hk.Linear(d_model)], name="importance_net"
        )

    def __call__(self, features: jnp.ndarray, output_gradients: jnp.ndarray) -> jnp.ndarray:
        """
        ...
        importance_input = jnp.concatenate([sensitivity * output_gradients, features], axis=-1)
...
def create_continual_learning_optimizer(
    base_lr: float = 1e-4, weight_decay: float = 0.01

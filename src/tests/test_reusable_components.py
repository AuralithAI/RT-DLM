...
class TestReusableAttention(unittest.TestCase):
    ...
    def test_attention_forward_pass(self):
        ...
        # Create input
        inputs = jax.random.normal(self.rng, (self.batch_size, self.seq_length, self.d_model))
    ...
    def test_attention_with_config(self):
        ...
        inputs = jax.random.normal(self.rng, (self.batch_size, self.seq_length, 128))
...
class TestPruningManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.manager = PruningManager(
            num_heads=self.num_heads, d_model=self.d_model, head_threshold=0.01, ffn_threshold=0.01
        )

    def test_head_usage_computation_3d(self):
        """Test head usage computation from 3D attention weights."""
        # Shape: [batch, seq, d_model]
        attn_weights = jax.random.uniform(jax.random.PRNGKey(0), (2, 8, self.d_model))
    ...
    def test_head_usage_computation_4d(self):
        ...
        attn_weights = jax.random.uniform(jax.random.PRNGKey(0), (2, self.num_heads, 8, 8))

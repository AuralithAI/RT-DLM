...
from src.core.model_parallel import (
    DeviceMesh,
...
class TestModelParallelConfig:
    ...
    def test_config_mesh_shape(self):
        ...
        config = ModelParallelConfig(mesh_shape=(1,), mesh_axis_names=("data",))
    ...
    def test_memory_optimization_flags(self):
        ...
        config = ModelParallelConfig(activation_checkpointing=True, offload_to_cpu=True)
        assert config.activation_checkpointing is True
        assert config.offload_to_cpu is True

    def test_communication_flags(self):
        """Test communication optimization flags"""
        config = ModelParallelConfig(async_communication=False, gradient_compression=True)
...
class TestDeviceMesh:
    ...
    def test_mesh_creation(self):
        ...
        config = ModelParallelConfig(tensor_parallel_size=1, pipeline_parallel_size=1)
...
class TestTensorParallelAttention:
    ...
    def test_attention_creation(self):
        ...
        def forward(x):
            attn = TensorParallelAttention(d_model=64, num_heads=4, mesh=mesh, name="tp_attn")
            return attn(x)
...
class TestTensorParallelMLP:
    ...
    def test_mlp_creation(self):
        ...
        def forward(x):
            mlp = TensorParallelMLP(d_model=64, d_ff=256, mesh=mesh, name="tp_mlp")
            return mlp(x)

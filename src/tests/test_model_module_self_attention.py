...
class TestSelfAttentionModel(unittest.TestCase):
    ...
    def test_mqa_attention(self):
        ...
        def init_fn():
            model = SelfAttentionModel(
                d_model=64, num_heads=4, vocab_size=1000, max_seq_length=128, attention_type="mqa"
            )
            x = jnp.zeros((1, 10), dtype=jnp.int32)
            return model(x)
...
class TestForwardPass(unittest.TestCase):
    ...
    def test_output_shape(self):
        ...
        def forward_fn(x):
            model = SelfAttentionModel(d_model=64, num_heads=4, vocab_size=1000, max_seq_length=128)
            return model(x)

...
from src.core.model.memory_bank import PersistentLTMStorage, MemoryBank
...
class TestPersistentLTMStorage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.d_model = 64
        self.storage = PersistentLTMStorage(d_model=self.d_model, storage_dir=self.test_dir)
    ...
    def test_store_single_memory(self):
        ...
        memory_id = self.storage.store_ltm(
            embedding, context="test memory", importance_score=0.8, emotional_valence=0.5
        )
    ...
    def test_retrieve_similar_memories(self):
        ...
        # Store the original
        self.storage.store_ltm(base_embedding, context="original", importance_score=1.0)

        # Store a similar one (slightly perturbed)
        similar = base_embedding + np.random.randn(self.d_model).astype(np.float32) * 0.1
        self.storage.store_ltm(similar, context="similar", importance_score=0.9)

        # Store some random ones
        for _ in range(5):
            random_emb = np.random.randn(self.d_model).astype(np.float32)
            self.storage.store_ltm(random_emb, context="random", importance_score=0.5)
    ...
    def test_save_and_load_cycle(self):
        ...
        new_storage = PersistentLTMStorage(d_model=self.d_model, storage_dir=self.test_dir)
        new_storage.load()
    ...
    def test_persistence_across_sessions(self):
        ...
        self.storage.store_ltm(known_embedding, context="known", importance_score=1.0)
        self.storage.save()
    ...
    def test_access_count_tracking(self):
        ...
        embedding = np.random.randn(self.d_model).astype(np.float32)
        self.storage.store_ltm(embedding, context="access_test", importance_score=1.0)
    ...
    def test_statistics(self):
        ...
        for i in range(3):
            embedding = np.random.randn(self.d_model).astype(np.float32)
            self.storage.store_ltm(embedding, importance_score=0.5 + i * 0.2, context=f"stat_{i}")
...
class TestMemoryBankWithPersistence(unittest.TestCase):
    ...
    def test_store_to_ltm_via_memory_bank(self):
        ...
        memory_id = self.memory_bank.store_ltm(embedding, context="via_bank", importance_score=0.9)
    ...
    def test_retrieve_from_ltm_via_memory_bank(self):
        ...
        # Store a memory
        embedding = np.random.randn(self.d_model).astype(np.float32)
        self.memory_bank.store_ltm(embedding, context="retrieve_test", importance_score=0.9)
    ...
    def test_save_and_load_via_memory_bank(self):
        ...
        for i in range(3):
            embedding = np.random.randn(self.d_model).astype(np.float32)
            self.memory_bank.store_ltm(embedding, context=f"bank_persist_{i}", importance_score=0.8)
    ...
    def test_memory_bank_without_persistence(self):
        ...
        non_persistent_bank = MemoryBank(
            memory_size=256, embedding_dim=self.d_model, retrieval_k=5, enable_persistent_ltm=False
        )
...
class TestPersistenceEdgeCases(unittest.TestCase):
    ...
    def test_load_without_save(self):
        ...
        storage = PersistentLTMStorage(d_model=self.d_model, storage_dir=self.test_dir)
    ...
    def test_large_number_of_memories(self):
        ...
        for i in range(num_memories):
            embedding = np.random.randn(self.d_model).astype(np.float32)
            storage.store_ltm(embedding, importance_score=np.random.random(), context=f"large_{i}")
    ...
    def test_repeated_save_cycles(self):
        ...
        # Third cycle - verify
        final_storage = PersistentLTMStorage(d_model=self.d_model, storage_dir=self.test_dir)
        final_storage.load()
...
class TestSecurityFeatures(unittest.TestCase):
    ...
    def test_filter_by_hashed_session(self):
        ...
        for _, metadata, _ in results:
            ...
            self.assertEqual(metadata["session_id"], storage._hash_id("session_a"))

    def test_encryption_enabled(self):
        """Test that encryption works when enabled."""
        storage = PersistentLTMStorage(
            d_model=self.d_model, storage_dir=self.test_dir, encryption_key="my_secret_password_123"
        )
    ...
    def test_pii_scrubbing_disabled(self):
        ...
        storage.store_ltm(embedding, context=f"Contact {email}", importance_score=0.8)

...
def create_real_batches(config: TrainingConfig, num_batches: int, rng_key: jax.random.PRNGKey):
    ...
    for _ in range(num_batches):
        ...
        input_ids = jax.random.randint(
            subkey, (config.batch_size, config.max_seq_length), minval=1, maxval=config.vocab_size
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            checkpoint = {
                ...
                "opt_state": jax.tree_util.tree_map(
                    lambda x: np.array(x) if isinstance(x, jnp.ndarray) else x, opt_state
                ),
                "step": 3,
                "config": {
                    "d_model": config.d_model,
...
class TestTrainScriptIntegration:
    ...
    def test_synthetic_batch_creation(self, config):
        ...
        try:
            from src.train import create_synthetic_batches

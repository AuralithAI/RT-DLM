...
class TestComputeController:
    ...
    def controller_fn(self):
        ...
        def forward(state_hidden, state_memory, state_uncertainty, state_confidence):
            controller = ComputeController(d_model=d_model, max_steps=10, halt_threshold=0.8)
...
class TestComputePlan:
    ...
    def test_deterministic_gate_parameters(self):
        ...
        for i in range(max_steps):
            assert f"update_gate_{i}_w" in compute_plan_params, f"Missing weight param for gate {i}"
            assert f"update_gate_{i}_b" in compute_plan_params, f"Missing bias param for gate {i}"
...
class TestComputeControllerConfig:
    ...
    def test_module_cost_config(self):
        ...
        costs = ModuleCostConfig(memory_retrieval=0.10, graph_reasoning=0.20)
...
class TestControllerLossComputer:
    ...
    def test_utilization_loss(self):
        ...
        loss_computer = ControllerLossComputer(lambda_utilization=0.1, target_utilization=0.3)

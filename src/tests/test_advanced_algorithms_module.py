...
class TestTaskMemory(unittest.TestCase):
    ...
    def test_task_memory_params(self):
        ...
        params = {"layer1": jnp.ones((5, 5)), "layer2": jnp.zeros((3, 3))}
...
class TestFisherInformation(unittest.TestCase):
    ...
    def test_compute_fisher_information_structure(self):
        ...
        params = {"w": jax.random.normal(jax.random.PRNGKey(0), (10, 5)), "b": jnp.zeros(5)}
...
class TestEWCLoss(unittest.TestCase):
    ...
    def test_ewc_loss_computation(self):
        ...
        try:
            ...
            params_star = {"w": jnp.ones((5, 5)), "b": jnp.zeros(5)}

            # New params (current)
            params = {"w": jnp.ones((5, 5)) * 1.1, "b": jnp.ones(5) * 0.1}

            # Fisher diagonal
            fisher_matrix = {"w": jnp.ones((5, 5)) * 100, "b": jnp.ones(5) * 50}

            ewc_loss = compute_ewc_loss(
                params=params, params_star=params_star, fisher_matrix=fisher_matrix, lambda_ewc=1.0
            )
...
class TestContinualLearner(unittest.TestCase):
    ...
    def test_learner_initialization(self):
        ...
        def forward(x):
            learner = ContinualLearner(d_model=64, lambda_ewc=400.0, lambda_si=1.0, max_tasks=5)
    ...
    def test_lambda_scaling(self):
        ...
        try:
            ...
            loss_high = compute_ewc_loss(
                params=params, params_star=params_star, fisher_matrix=fisher_matrix, lambda_ewc=10.0
            )

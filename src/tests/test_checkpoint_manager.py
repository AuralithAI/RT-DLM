...
class TestFlattenParams(unittest.TestCase):
    ...
    def test_flatten_simple_params(self):
        ...
        params = {
            "layer1": {"weight": jnp.ones((10, 10)), "bias": jnp.zeros(10)},
            "layer2": {"weight": jnp.ones((5, 10)), "bias": jnp.zeros(5)},
        }
...
class TestUnflattenParams(unittest.TestCase):
    ...
    def test_unflatten_params(self):
        ...
        original = {"layer1": {"weight": jnp.ones((10, 10)), "bias": jnp.zeros(10)}}
...
class TestCheckpointManager(unittest.TestCase):
    ...
    def test_save_checkpoint(self):
        ...
        manager = CheckpointManager(checkpoint_dir=str(self.checkpoint_dir), model_name="save_test")

        params = {"layer": {"weight": jnp.ones((10, 10)), "bias": jnp.zeros(10)}}
        ...
        path = manager.save_checkpoint(
            params=params, opt_state=opt_state, epoch=5, step_count=1000, metrics={"loss": 0.1}
        )
    ...
    def test_load_checkpoint(self):
        ...
        # Save
        path = manager.save_checkpoint(params=params, opt_state=opt_state, epoch=3, step_count=500)

        # Load
        checkpoint = manager.load_checkpoint(checkpoint_path=path, load_opt_state=False)
    ...
    def test_load_latest_checkpoint(self):
        ...
        for epoch in [1, 2, 3]:
            manager.save_checkpoint(params=params, opt_state=opt_state, epoch=epoch)
...
class TestCheckpointWithMetrics(unittest.TestCase):
    ...
    def test_save_with_training_history(self):
        ...
        manager = CheckpointManager(checkpoint_dir=self.temp_dir, model_name="history_test")
...
class TestCheckpointExtraTensors(unittest.TestCase):
    ...
    def test_save_extra_tensors(self):
        ...
        manager = CheckpointManager(checkpoint_dir=self.temp_dir, model_name="extra_test")

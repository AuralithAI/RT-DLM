...
class TestHybridRetriever:
    ...
    def test_search_hybrid(self, retriever, sample_documents, sample_embeddings):
        ...
        results = retriever.search("neural networks", query_embedding=query_embedding, top_k=3)

        assert len(results) <= 3

    def test_clear(self, retriever, sample_documents, sample_embeddings):
        """Test clearing the retriever."""
        retriever.add_documents(documents=sample_documents, embeddings=sample_embeddings)
...
class TestDocumentIngester:
    ...
    def ingester(self):
        ...
        return DocumentIngester(chunk_size=100, chunk_overlap=20, embedding_dim=64)

    @pytest.fixture
    def sample_long_document(self):
        return " ".join(["This is sentence number {}.".format(i) for i in range(50)])
...
class TestModelEmbeddingIntegration:
    ...
    def test_schedule_retrieval_update(self, config):
        ...
        trainer = AGITrainer(config)
        trainer.configure_retrieval(RetrievalConfig.for_training(), documents=["Test doc."])

...
from src.config.rlm_config import PartitionStrategy
...
Line 5"""
    ...
    def test_parent_child_relationship(self):
        store = ContextStore()
        store.store("parent", "Parent content that is long enough to partition")

        chunk_names = store.partition("parent", chunk_size=10, overlap=2)

        assert len(chunk_names) > 1, "Should create multiple chunks"
        for name in chunk_names:
            var = store.get(name)
            assert var is not None

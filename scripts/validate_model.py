...
def validate_model_init(preset: str):
    ...
    sample_input = jax.random.randint(rng, (batch_size, seq_length), 0, config.vocab_size)
...
def validate_training_step(preset: str):
    ...
    input_ids = jax.random.randint(rng, (batch_size, seq_length), 0, config.vocab_size)
    targets = jax.random.randint(rng, (batch_size, seq_length), 0, config.vocab_size)
    ...
    has_nan_grad = any(jnp.any(jnp.isnan(g)) for g in jax.tree_util.tree_leaves(grads))
    if has_nan_grad:
    ...
    changed = any(not jnp.allclose(o, n) for o, n in zip(old_flat, new_flat))
...
def main():
    parser = argparse.ArgumentParser(description="Validate RT-DLM model and training readiness")

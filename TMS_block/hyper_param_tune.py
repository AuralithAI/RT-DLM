...
def objective(trial):
    ...
    audio_sample_rate = trial.suggest_categorical("audio_sample_rate", [16000, 22050])
    ...
    trial_params = {
        ...
        "weight_decay": 1e-3,
        "clip_norm": 0.5,
        "max_sentence_length": 5192,
        "input_sentence_size": 500000,
        "character_coverage": 0.9999,
        "num_threads": 16,
        "xla_gpu_parallelism": 10,
        "EPSILON": 1e-8,
    ...
    logger.info(f"[Trial {trial.number}] d_model: {d_model}, num_layers: {num_layers}, audio_sample_rate: {audio_sample_rate}")
    ...
    logger.info(f"[INFO] Saved params, state, memory banks, and thought_log for trial {trial_number}")
    ...
    plt.close()
    logger.info(f"[INFO] Memory retrieval similarity plot saved for trial {trial_number}")
...
if __name__ == "__main__":
    ...
    logger.info("[INFO] Best model parameters saved as TMS_block/tms_best_params.pkl")
    logger.info("[INFO] Best state saved as TMS_block/tms_best_state.pkl")
    logger.info("[INFO] Best LTM bank saved as TMS_block/ltm_bank.pkl")
    logger.info("[INFO] Best STM bank saved as TMS_block/stm_bank.pkl")
    logger.info("[INFO] Best MTM bank saved as TMS_block/mtm_bank.pkl")
    logger.info("[INFO] Best loss plot saved as TMS_block/tms_best_loss.png")